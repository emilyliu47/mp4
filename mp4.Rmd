...
```{r}
#station_summary table from database
station_summary <- db %>%
  dbGetQuery("SELECT station_id, name, lat, lon
             FROM station_summary;")

top_10_visits <- function(demographic, demo_type) { #this function takes in a demographic type and outputs one table that is the 10 most visited (start and end) stations for the demographic

#number of female visits to each start station
start_tally <- db %>%
  dbGetQuery(paste("
...
#number of female visits to each end station
end_tally <- db %>%
  dbGetQuery(paste("
